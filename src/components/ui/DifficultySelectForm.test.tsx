...
describe("DifficultySelectForm", () => {
  it("난이도를 고르기 전에는 체험 시작하기 버튼이 비활성 상태다", () => {
    render(<DifficultySelectForm onComplete={vi.fn()} />);

    expect(screen.getByRole("button", { name: "체험 시작하기" })).toBeDisabled();
  });
  ...
  it("카드를 고른 뒤 체험 시작하기를 누르면 onComplete가 그 id로 정확히 한 번 호출된다", () => {
    const onComplete = vi.fn();
    render(<DifficultySelectForm onComplete={onComplete} />);

    fireEvent.click(screen.getByRole("button", { name: /중간/ }));

    const startButton = screen.getByRole("button", { name: "체험 시작하기" });
