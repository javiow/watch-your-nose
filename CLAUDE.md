...
## 아키텍처 규칙
- CRITICAL: DB를 두지 않는다. 모든 콘텐츠(보이스피싱 시나리오, 사례 쌍, 매물 쌍, 대응 방안 카피)는 `src/data/`의 정적 TS 파일로만 관리한다. 세션/사용자 데이터를 저장하는 새 백엔드 리소스를 추가하지 않는다.
- 예외: `src/app/api/npc-chat/route.ts`는 케이스 조사(2단계) NPC 자유 입력 질문을 사전 정의된 statement 중 하나로 "분류"만 하는 상태 없는(stateless) 라우트다. LLM은 답변 문장을 직접 생성하지 않으며, 항상 `src/data/`에 미리 써둔 curated 텍스트만 반환한다 — 스포일러·환각 위험이 없다. `ANTHROPIC_API_KEY`가 없거나 호출이 실패하면 `src/lib/npc-chat.ts`의 정적 키워드 매칭으로 자동 폴백하므로, 이 라우트가 없어도 앱은 완전히 동작한다.
...
- CRITICAL: 시크릿·API 키는 코드에 하드코딩하지 않는다. `.env*`(gitignore 대상, `.env.example`만 예외)로만 다룬다. `ANTHROPIC_API_KEY`는 `src/app/api/npc-chat/route.ts`에서만 서버 사이드로 읽으며, 클라이언트 코드나 API 응답으로 노출하지 않는다.
