...
- **단계 수 권장**: `src/data/voice-phishing.ts`의 시나리오는 3~5턴짜리 대화 그래프로 만들어지므로,
  `typical_flow`도 가능하면 3~5단계로 서술해 생성 에이전트가 노드로 바로 옮기기 쉽게 한다(기존
  항목을 소급 수정할 필요는 없음, 신규 조사분부터 적용).
