...
2. 기관명·상호명은 "OO은행", "OO쇼핑" 같은 익명화된 형태로만 쓴다. 실존 기관/기업의 실명을
   가해자로 등장시키지 않는다.
3. 사용자(`me`)가 고를 수 있는 선택지는 항상 "정보를 알려준다/따른다" 계열과 "거절한다" 계열의
   이지선다 구조를 유지하고, **정답은 반드시 `id`가 `refuse`로 시작하는 선택지 하나**여야 한다
   (`isCorrect = isNormalCase ? !refused : refused`이며 이 시나리오는 `isNormalCase: false`이므로
   거절이 정답). comply 계열 선택지는 오답으로만 이어지고, 그 자체가 실행 가능한 사기 절차 안내가
   되지 않게 한다(예: 원격앱 설치를 요구하더라도 앱 이름·설치 방법을 구체적으로 쓰지 않는다).
4. 사례 뱅크에서 최소 1개 이상의 항목을 근거로 삼는다. 참고한 항목의 `[id]`를 작업 요약에 인용한다.
   근거 없는 순수 창작은 하지 않는다.
5. 신규 시나리오 `id`는 `scam-<slug>` 형식이며, 기존 `voice-phishing.ts`의 모든 id와 중복되지
   않아야 한다.
6. 시나리오 종료 시점(선택지 이후 `next`가 없는 노드)까지 이어지는 대화 그래프를 만들고, 모든
   노드가 1개 이상의 선택지를 갖도록 한다(`startNodeId`가 실제 노드에 존재해야 함).
