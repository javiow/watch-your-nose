...
3. **정답 로직(이번 개편의 핵심 규칙)**: 선택지마다 `risk: "safe" | "caution" | "danger"`를
   명시한다. 체험 참가자는 이미 이것이 보이스피싱 교육 콘텐츠라는 걸 알고 들어오기 때문에, 정상
   케이스에서도 반사적으로 의심하고 끊거나 무시하려는 경향이 있다. **듣지도/확인하지도 않고 그냥
   끊거나 무시하는 선택지는 첫 번째 노드를 포함해 어느 노드에서 나오든 반드시 `risk: "danger"`
   (항상 종료)로 태깅한다.** `risk: "safe"`는 (a) 요청에 정상적으로 협조하거나 (b) "미심쩍으면
   카드 뒷면·공식 앱 등 공식 채널로 직접 재확인하겠다"처럼 **근거를 댄 확인 절차**를 선택했을
   때만 붙인다. 단순 의심만으로 끊는 것과 확인 절차를 거쳐 판단하는 것을 같은 등급으로 취급하지
   않는다. `caution`은 근거 있는 확인은 아니지만 완전한 무시도 아닌 회색지대 대응(예: 제대로
   확인 안 하고 대충 얼버무려 넘어감)에 붙인다.
...
6. 노드 체인은 `startNodeId`부터 최장 경로 기준 **3~5개**여야 하고(3~5턴 인터랙티브 대화), 그 중
   최소 1개 노드에는 `caution` 분기(회색지대 선택지)를 포함한다. 모든 노드가 1개 이상의 선택지를
   갖고, `startNodeId`가 실제 노드에 존재해야 한다.
...
2. 시나리오 개수/카테고리 검증 등 하드코딩된 assertion이 있다면 추가하려는 시나리오 수만큼 먼저
   Edit로 갱신한다. 노드 체인 길이(3~5)·`caution`은 `next` 필수·`danger`는 `next` 금지·시작
   노드에 `danger`-terminal이 있는지(반사적 거부 함정)를 검증하는 assertion도 이미 있는지 확인한다.
...
- **반사적 거부 선택지가 첫 노드를 포함해 모든 노드에서 `risk: "danger"`로 태깅됐는지 확인**
  (근거 있는 확인 절차만 `safe`)
- id 중복 없음 확인
- red → green 테스트 로그
- lint 통과 여부
