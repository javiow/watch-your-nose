...
1. `src/data/remediation.ts`: `mistakeTag`별 대응 방안 초안 카피를 작성한다 (`blind-refusal`, `missed-scam-signal`, `missed-lease-fraud-signal` 각각에 대해 — 왜 틀렸는지 설명 + 실생활 대응 팁 2~3줄). 나중에 쉽게 교체할 수 있도록 이 데이터 파일에만 문구를 둔다. `mistakeTag`로 조회했을 때 매핑이 없는 경우(오타 등으로 키가 어긋난 경우)를 대비해 일반적인 기본 안내 문구도 하나 정의해, 조회 실패 시 빈 화면 대신 그 문구를 보여준다.
...
4. `src/app/session/page.tsx` (`"use client"`):
   ...
   - 상단에 진행률 바("N/3", 유형명 노출 금지) 표시. 모바일 폭에서도 카드/선택지가 세로로 자연스럽게 쌓이도록 `docs/UI_GUIDE.md`의 반응형 규칙을 따른다.
   - `onComplete(result)`에서 `addResult(result)` 후 다음 단계로 이동한다. 이 전환 중에는 다음 렌더로 넘어가기 전까지 같은 단계에서 "다음"이 다시 눌려도 `addResult`가 중복 호출되지 않게 막는다(예: 전환 중 상태 플래그로 재클릭 무시). 마지막 단계 완료 시 `/result`로 이동.
   - `sessionPlan`이 비어있으면(직접 URL 진입 등) `/`로 리다이렉트한다.
   - 세션 도중 사용자가 브라우저 뒤로가기 등으로 이탈하는 것은 막지 않는다(경고창 없음) — `docs/ARCHITECTURE.md` 엣지 케이스 절 참고.

5. `src/app/result/page.tsx` (`"use client"`):
   - `useSession().results`로 `aggregateResults()`(step1의 `scoring.ts`) 호출해 종합 점수/등급을 표시한다. 화면에 보여줄 점수는 정수(%)로 반올림한다.
   - 문항별 상세 리뷰: 각 `ModuleResult`의 `userChoice` vs `correctChoice`, `explanation`을 리스트로 보여준다.
   - `isCorrect === false`인 결과들의 `mistakeTag`를 모아 `remediation.ts`에서 대응 방안을 찾아 렌더한다. 매핑이 없는 태그는 기본 안내 문구로 대체한다(1번 항목 참고).
   - CTA는 **"다시 체험하기" 하나만** 제공한다 — 클릭 시 `resetSession()` 후 랜딩을 거치지 않고 바로 `/session`으로 이동. "처음으로/홈" 버튼은 만들지 않는다. 재시도 시 콘텐츠는 직전 콘텐츠를 제외하지 않는 순수 랜덤이다(의도된 동작).
   - `results.length`가 `EXPERIENCE_MODULES.length`(등록된 유형 수)와 정확히 같을 때만 렌더한다 — 0개든 일부만 완료된 상태든(직접 URL 진입 등) `/`로 리다이렉트한다.
...
2. `npm run dev`로 로컬 실행 후 수동으로 확인한다:
   ...
   - `/result`에 세션 없이 직접 URL로 진입하면 `/`로 리다이렉트되는가? (0개, 1~2개 부분 완료 상태 모두 확인)
   - 브라우저 폭을 모바일 크기로 줄여도 레이아웃이 깨지지 않고 카드/선택지가 세로로 자연스럽게 배치되는가?
...
- 세션 도중 이탈(뒤로가기, 탭 닫기 등)을 막는 `beforeunload` 확인창이나 라우터 가드를 추가하지 마라. 이유: 제품 결정 — 경고 없이 그대로 허용.
- "다시 체험하기"에서 직전 콘텐츠를 제외하는 로직을 추가하지 마라. 이유: 제품 결정 — 순수 랜덤 유지.
- 랜딩 카피, 문항별 리뷰의 `explanation`, `remediation.ts` 대응방안 문구를 렌더링할 때 `dangerouslySetInnerHTML`을 쓰지 마라. 이유: XSS 방지, `CLAUDE.md` 보안 규칙.
- 기존 테스트를 깨뜨리지 마라.
